...
// Maps store tab IDs to dock items
const DOCK_ITEMS = [
  { id: 'feed', icon: Home, label: 'Feed' },
  { id: 'graph', icon: Share2, label: 'Graph' },
  { id: 'create', icon: Plus, label: 'Create' },
  { id: 'chat', icon: MessageCircle, label: 'Chat' }, // mapped 'assistant' to 'chat'
...
export function LiquidDock() {
  ...
  return (
    ...
    >
      <div className="liquid-glass-dock rounded-full px-3 py-2 flex items-center gap-1">
        {DOCK_ITEMS.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Label tooltip */}
              <span className="absolute -top-10 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/80 backdrop-blur-md rounded-md text-[10px] text-white/80 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none border border-white/10">
                {item.label}
              </span>
            </motion.button>
          );
